...
[assembly:
    MelonInfo(typeof(EditPlayerData.EditPlayerData), ModHelperData.Name, ModHelperData.Version,
...
public class EditPlayerData : BloonsTD6Mod
{
    ...
    {
        var versions = Directory.GetDirectories(@"C:\Windows\Microsoft.NET\Framework64\")
            .Where(f => f.Split('\\')[^1][0] == 'v').ToList();
        versions.Sort();
...
// ReSharper disable once InconsistentNaming
    internal static class SettingsScreen_Open
    {
        ...
        internal static void Postfix(SettingsScreen __instance)
        {
            ...
            var button = panel.AddModHelperPanel(new Info("Edit Data Button"), null, RectTransform.Axis.Vertical, 10);
            ...
            buttons.AddButton(new Info("Import", 285), VanillaSprites.EditBtn, new Action(() =>
            {
                ...
                try
                {
                    ...
                    PopupScreen.instance.SafelyQueue(screen => { screen.ShowOkPopup("Settings successfully loaded from file."); });
                }
                catch
                {
                    PopupScreen.instance.SafelyQueue(screen =>
                    {
                        screen.ShowOkPopup("An error occured while loading. Check the MelonLoader console for messages and ensure the file is in the proper format.");
                    });
                    throw;
                }
            }));

            buttons.AddButton(new Info("Settings", 285), VanillaSprites.SettingsBtn, new Action(() =>
            {
                ModGameMenu.Open<EditPlayerDataMenu>();
            }));

            buttons.AddButton(new Info("Export", 285), VanillaSprites.ExitGameBtn, new Action(() =>
            {
                ...
                try
                {
                    ...
                    PopupScreen.instance.SafelyQueue(screen => { screen.ShowOkPopup("Settings successfully saved to file."); });
                }
                catch
                {
                    PopupScreen.instance.SafelyQueue(screen =>
